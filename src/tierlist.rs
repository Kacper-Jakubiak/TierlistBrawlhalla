...
pub fn verify_weapon_tierlist(weapons_tierlist: &[Vec<String>], weapons: &[String]) -> Result<(), String> {
    let tier_set: HashSet<&str> = weapons_tierlist.iter().flatten().map(String::as_str).collect();
...
pub fn create_legend_tierlist(data: (HashMap<String, (String, String)>, Vec<Vec<String>>)) -> Result<Vec<Vec<String>>, String> {
    let (legend_map, weapons_tierlist) = data;
    ...
    verify_weapon_tierlist(&weapons_tierlist, &weapons_vec)?;
    ...
    let mut legend_tierlist: Vec<Vec<String>> =
    ...
    for (legend, (w1, w2)) in legend_map {
        ...
        legend_tierlist[legend_tier].push(legend);
    }
